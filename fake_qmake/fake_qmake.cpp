...
for(i=1; i<=n_sources ; i++)
{
  if(i==n_sources) fprintf(fout,"\t%s.o\n\n",getName(row_sources.text(i)));
  else             fprintf(fout,"\t%s.o \\\n",getName(row_sources.text(i)));
}  
fprintf(fout,"DIST          = \n");
fprintf(fout,"QMAKE_TARGET  = %s\n", str_target.text());
fprintf(fout,"DESTDIR        =  #avoid trailing-slash linebreak\n");
...
if(     strcmp(str_template.text(),"app") == 0)                      fprintf(fout,"DESTDIR_TARGET         = %s.exe\n", str_target.text());
else if(strcmp(str_template.text(),"lib") == 0 && is_staticlib)      fprintf(fout,"DESTDIR_TARGET         = lib%s.a\n", str_target.text());
else if(strcmp(str_template.text(),"lib") == 0 && is_staticlib == 0) fprintf(fout,"DESTDIR_TARGET         = %s.dll\n", str_target.text());
fprintf(fout,"\n");
...
fprintf(fout,"\t@$(QMAKE) -o %s %s\n", fmake.text(), fname.text());
fprintf(fout,"\n");
fprintf(fout,"clean:\n"); 
fprintf(fout,"\t-$(DEL_FILE)");
for(i=1; i<=n_sources ; i++)
{
  if(i==n_sources) fprintf(fout," %s.o\n\n",getName(row_sources.text(i)));
  else             fprintf(fout," %s.o \\\n",getName(row_sources.text(i)));
...
for(i=1; i<=n_sources; i++)
{
  fprintf(fout,"%s.o: %s", getName(getName(row_sources.text(i))), row_sources.text(i));
  printDepend(i);
  fprintf(fout,"\n");
  if(strcmp(getExtension(row_sources.text(i)),".c") == 0)
  {
    fprintf(fout,"\t$(CC) -c $(CFLAGS) $(INCPATH) -o %s.o %s\n", getName(row_sources.text(i)), row_sources.text(i));
  }
  else
  {
    fprintf(fout,"\t$(CXX) -c $(CXXFLAGS) $(INCPATH) -o %s.o %s\n", getName(row_sources.text(i)), row_sources.text(i));
  }
  fprintf(fout,"\n");
...
fprintf(fout,"# Template: %s\n", str_template.text());
fprintf(fout,"# Command: fake_qmake -o %s %s\n", fmake.text(), fname.text());
fprintf(fout,"#########################################################################################\n");
fprintf(fout,"\n");
fprintf(fout,"####### Compiler, tools and options\n");
fprintf(fout,"\n");
...
for(i=1; i<=n_sources; i++)
{
  fprintf(fout,"\t\t%s.o", getName(row_sources.text(i)));
  if(i==n_sources) fprintf(fout,"\n\n");
  else             fprintf(fout," \\\n");
}
if     (strcmp(str_template.text(),"app") == 0)
...
fprintf(fout,"####### Build rules\n");
fprintf(fout,"\n");
fprintf(fout,"all: Makefile $(TARGET)\n");
fprintf(fout,"\n");
if     (strcmp(str_template.text(),"app") == 0)
{
  fprintf(fout,"$(TARGET):  $(OBJECTS)\n");  
  fprintf(fout,"\t$(LINK) $(LFLAGS) -o $(TARGET) $(OBJECTS) $(OBJCOMP) $(LIBS)\n");
}
else if(strcmp(str_template.text(),"lib") == 0)
{
  int staticlib_found = 0;
  ...
  {
    fprintf(fout,"$(TARGET):  $(OBJECTS)\n");  
    fprintf(fout,"\t-$(DEL_FILE) $(TARGET) $(TARGET0) $(TARGET1) $(TARGET2)\n");
    fprintf(fout,"\t$(LINK) $(LFLAGS) -o $(TARGET) $(OBJECTS) $(LIBS) $(OBJCOMP)\n");
    fprintf(fout,"\t-ln -s $(TARGET) $(TARGET0)\n");
    fprintf(fout,"\t-ln -s $(TARGET) $(TARGET1)\n");
    fprintf(fout,"\t-ln -s $(TARGET) $(TARGET2)\n");
    fprintf(fout,"\n");
  }  
  fprintf(fout,"\n");
  fprintf(fout,"staticlib: $(TARGETA)\n");
  fprintf(fout,"\n");
  fprintf(fout,"$(TARGETA):  $(OBJECTS) $(OBJCOMP)\n");
  fprintf(fout,"\t-$(DEL_FILE) $(TARGETA)\n");
  fprintf(fout,"\t$(AR) $(TARGETA) $(OBJECTS)\n");
...
for(i=1; i<=n_sources; i++)
{
  fprintf(fout,"%s.o: %s", getName(row_sources.text(i)), row_sources.text(i));
  printDepend(i);
  fprintf(fout,"\n");
  if(strcmp(getExtension(row_sources.text(i)),".c") == 0)
  {
