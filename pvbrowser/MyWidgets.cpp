...
bool MyQWidget::event(QEvent *event)
{
  ...
  if(event->type() == QEvent::Gesture)
  {
    ...
    if(QGesture *swipe = ge->gesture(Qt::SwipeGesture))
    {
      ...
      {
        strcat(buf,"finished");
        QSwipeGesture *sg = (QSwipeGesture *) ge;
        if(sg->horizontalDirection() == QSwipeGesture::Left || sg->verticalDirection() == QSwipeGesture::Up)
        {
          
        }
      }
    }
    else if(QGesture *pan = ge->gesture(Qt::PanGesture))
    {
      strcat(buf,"pan ");
      MyScrollArea *sa = NULL;
      if(parent() != NULL) sa = (MyScrollArea *) parent();
      if(sa != NULL)
      {
        //QScrollBar *hsb = sa->horizontalScrollBar();
        //QScrollBar *vsb = sa->verticalScrollBar();
        QPanGesture *pg = (QPanGesture *) pan;
        QPointF point = pg->delta();
        if(point.x() > 0)
        {
          //QKeyEvent kbevent(QEvent::KeyPress, Qt::Key_PageUp, Qt::NoModifier);
          //QCoreApplication::sendEvent(hsb, &kbevent);
          QKeyEvent kbevent(QEvent::KeyPress, Qt::Key_Right, Qt::NoModifier);
          QCoreApplication::sendEvent(sa, &kbevent);
        }
        else if(point.x() < 0)
        {
          //QKeyEvent kbevent(QEvent::KeyPress, Qt::Key_PageDown, Qt::NoModifier);
          //QCoreApplication::sendEvent(hsb, &kbevent);
          QKeyEvent kbevent(QEvent::KeyPress, Qt::Key_Left, Qt::NoModifier);
          QCoreApplication::sendEvent(sa, &kbevent);
        }
        if(point.y() > 0)
        {
          //QKeyEvent kbevent(QEvent::KeyPress, Qt::Key_PageUp, Qt::NoModifier);
          //QCoreApplication::sendEvent(vsb, &kbevent);
          QKeyEvent kbevent(QEvent::KeyPress, Qt::Key_Down, Qt::NoModifier);
          QCoreApplication::sendEvent(sa, &kbevent);
        }
        else if(point.y() < 0)
        {
          //QKeyEvent kbevent(QEvent::KeyPress, Qt::Key_PageDown, Qt::NoModifier);
          //QCoreApplication::sendEvent(vsb, &kbevent);
          QKeyEvent kbevent(QEvent::KeyPress, Qt::Key_Up, Qt::NoModifier);
          QCoreApplication::sendEvent(sa, &kbevent);
        }
      }
    }  
    if(QGesture *pinch = ge->gesture(Qt::PinchGesture))
    {
      ...
      int percent = 0;
      MainWindow *mw = NULL;
      if(parent() != NULL) mw = (MainWindow *) parent()->parent();
      if(mw != NULL)
      {
        QPinchGesture *pg = (QPinchGesture *) pinch;
        if(pg->scaleFactor() > pg->lastScaleFactor())
        {
          percent = mw->pvbtab[mw->currentTab].interpreter.percentZoomMask + 5; 
          if(percent > 250) percent = 250;
        }
        else if(pg->scaleFactor() < pg->lastScaleFactor())
        {
          percent = mw->pvbtab[mw->currentTab].interpreter.percentZoomMask - 5;
