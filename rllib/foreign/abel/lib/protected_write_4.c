...
extern struct _data protwrite4 (struct _comm comm1, char *addr, int count, 
			struct _data outdata, int mask, int plctype, int debug)
{
...
int x,place,d,location,hlen;
	...
	df1_1.data[place++] = (mask & 255);
	df1_1.data[place++] = (mask / 256);
