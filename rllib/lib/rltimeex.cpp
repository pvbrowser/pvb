...
void rlTimeEx::normalizeAsRelativeTime()
{
...
{
  assert(
      not objectHoldsRelativeTime
          && "object must represent absolute time and date, otherwise normalization as time and date is not defined");
  assert(year >= 1970);
 ...
 * @param time_string
 */
rlTimeEx& rlTimeEx::setTimeFromString(const char *time_string)
{
  year = 0;
  month = 0;
  day = 0;
 ...
 * @param time_string
 */
rlTimeEx& rlTimeEx::setTimeFromIsoString(const char *iso_time_string)
{
  year = 0;
  month = 0;
  day = 0;
...
{
  assert(not objectHoldsRelativeTime && "object mus hold an absolute time and date");
...
const char *rlTimeEx::toString(const char *format)
{
  ...
  while (*format != '\0')
  {
    ...
    if (unsigned(dest - time_string + 6) > sizeof(time_string))
      break;
  }
  *dest = '\0';
  return time_string;
...
rlTimeEx& rlTimeEx::getLocalTime()
{
#ifdef RLUNIX
  time_t t;
  struct tm *tms;
...
rlTimeEx& rlTimeEx::operator+=(time_t seconds)
{
  if (0 > seconds)
...
rlTimeEx rlTimeEx::operator+(time_t seconds) const
                             {
  rlTimeEx t(*this);
...
rlTimeEx& rlTimeEx::operator +=(const ftime_t& seconds)
{
...
rlTimeEx rlTimeEx::operator +(const ftime_t& seconds) const
                              {
  rlTimeEx t(*this);
...
rlTimeEx& rlTimeEx::operator+=(const RelativeTime &t)
{
...
rlTimeEx rlTimeEx::operator+(const RelativeTime &time) const
                             {
  rlTimeEx t(*this);
...
double rlTimeEx::operator-(const rlTimeEx &time) const
                           {
...
const char* rlTimeEx::formatTimeDiff(ftime_t tdiff, enum FormatLargestUnit fmt, unsigned bufferLength, char* buffer)
{
  if (0 < bufferLength)
  {
    if (not buffer)
...
const char* rlTimeEx::formatTimeDiff(const rlTimeEx& t1, const rlTimeEx& t2, enum FormatLargestUnit fmt, unsigned bufferLength, char* buffer)
{
...
std::string rlTimeEx::formatTimeDiffString(const rlTimeEx& t1, const rlTimeEx& t2, enum FormatLargestUnit fmt)
{
