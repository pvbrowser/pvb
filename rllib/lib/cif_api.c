...
short               sRet = DRV_NO_ERROR;
 ...
 if ( hDevDrv == INVALID_HANDLE_VALUE) {
    ...
    if ( sRet == DRV_NO_ERROR) {
      ...
      tBuffer.pabInfoData   = (unsigned char *) pvData;   // needed in kernel-space to copy data back to it!
      tBuffer.sError        = sRet;
      // activate function
...
short               sRet = DRV_NO_ERROR;
 ...
 if ( hDevDrv == INVALID_HANDLE_VALUE) {
   sRet = DRV_USR_NOT_INITIALIZED;
    ...
    tBuffer.pabReadData  = (unsigned char *) pvData;
    tBuffer.sError       = sRet;
...
short             sRet = DRV_NO_ERROR;
 ...
 if ( hDevDrv == INVALID_HANDLE_VALUE) {
   sRet = DRV_USR_NOT_INITIALIZED;
    ...
    tBuffer.usBoard    = usDevNumber;
    tBuffer.usMode     = usMode;
    tBuffer.pabExtData = (unsigned char *) pvData;
    tBuffer.sError     = sRet;
...
short               sRet = DRV_NO_ERROR;
  ...
  if ( hDevDrv == INVALID_HANDLE_VALUE) {
    ...
    tBuffer.pabData  = (unsigned char *) pvData;
    tBuffer.sError   = sRet;
...
int               lRet = 0;
  ...
  if ( hDevDrv == INVALID_HANDLE_VALUE) {
    ...
    tBuffer.pabSendData     = (unsigned char *) pvSendData;
    tBuffer.pabReceiveData  = (unsigned char *) pvReceiveData;
    tBuffer.sError          = sRet;
    tBuffer.ulTimeout       = ulTimeout;
...
short openFile ( const char *fileName, FILEDATA *ptFile) 
{
  ...
  if ( ptFile->fd < 0 ) {
	...
	if ( (ptFile->pabBuffer = (char*)malloc( ptFile->lSize)) == NULL) {
	  ...
	  if ( (ssize_t) ( lNumberOfBytesRead = read( ptFile->fd,              // handle of file to read
